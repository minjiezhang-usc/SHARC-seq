...
This script can be used to split DG to different groups and calculate the distance of two arms of each DG.

First, we need get the base pair info from PDB database; (eg, 4V6X: Structure of the human 80S ribosome);
and the base pairing info should be stored in IGV arc format. This file will be used to identity the dsRNAs.

Second, we need get the information of corresponding nucleotide position in PDB and reference files. This file
is important to calculate the correct distance between two nucleotides.

eg:
python DG_identity.py  1HR2_pyr.sam  1HR2.arc.bed  1hr2.pdb  B  PDB_1HR2_ntpos.txt  5  40  0.8  ref.fa  1HR2  20201218
"""
...
if len(sys.argv) < 3:
    print("Usage:           python DG_identity.py sam_crssant  shape_bedpragh  DG_reads_cutoff  DGcommon_ratio  seqlen  extendlen  DG/reads  outputprefix")
    ...
    print("DG/reads:        DG or each reads was used for analysis")
    print("outputprefix                                     ")
    sys.exit()
...
print('totally analyzed DG/reads:\t'+str(len(newDGspan_filter)))
